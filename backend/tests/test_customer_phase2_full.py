"""
Test Customer Interface Phase 2 APIs (Full E2E):
- GET /api/auth/recommended-venues - Personalized venue recommendations based on preferences
...
class TestRecommendedVenuesEndpoint:
    """Tests for GET /api/auth/recommended-venues endpoint - NEW for Phase 2."""

    def test_recommended_venues_requires_auth(self):
        """Test that recommended-venues endpoint requires authentication."""
        response = requests.get(f"{BASE_URL}/api/auth/recommended-venues")
        assert response.status_code == 401, f"Expected 401, got {response.status_code}"
        print("PASS: recommended-venues requires authentication")

    def test_recommended_venues_returns_venues(self, auth_headers):
        """Test that recommended-venues returns a list of venues."""
        response = requests.get(f"{BASE_URL}/api/auth/recommended-venues", headers=auth_headers)
        assert response.status_code == 200, f"Expected 200, got {response.status_code}: {response.text}"
        
        data = response.json()
        assert "venues" in data, "Response should contain 'venues' key"
        assert "total" in data, "Response should contain 'total' key"
        assert "has_preferences" in data, "Response should contain 'has_preferences' key"
        assert isinstance(data["venues"], list), "venues should be a list"
        print(f"PASS: recommended-venues returns {data['total']} venues, has_preferences={data['has_preferences']}")

    def test_recommended_venues_has_preference_matching(self, auth_headers):
        """Test that recommended venues match user preferences (Delhi/Mumbai, Wedding)."""
        response = requests.get(f"{BASE_URL}/api/auth/recommended-venues", headers=auth_headers)
        assert response.status_code == 200
        
        data = response.json()
        if data["has_preferences"] and len(data["venues"]) > 0:
            # Check that venues are from preferred cities (Delhi/Mumbai)
            preferred_cities = ["Delhi", "Mumbai"]
            for venue in data["venues"][:5]:  # Check first 5
                assert venue.get("city") in preferred_cities, f"Venue {venue.get('name')} city {venue.get('city')} not in preferred cities"
            print(f"PASS: Recommended venues match preferred cities (Delhi/Mumbai)")
        else:
            print("SKIP: No preferences or no venues to verify matching")

    def test_recommended_venues_has_required_fields(self, auth_headers):
        """Test that recommended venues have all required display fields."""
        response = requests.get(f"{BASE_URL}/api/auth/recommended-venues", headers=auth_headers)
        assert response.status_code == 200
        ...
        if len(data["venues"]) > 0:
            venue = data["venues"][0]
            required_fields = ["venue_id", "name", "city", "images", "rating"]
            for field in required_fields:
                assert field in venue, f"Venue missing required field: {field}"
            print(f"PASS: Recommended venue has all required fields: {required_fields}")
        else:
            print("SKIP: No venues to verify fields")
...
class TestMyReviewsEndpoint:
    ...
    def test_my_reviews_has_venue_enrichment(self, auth_headers):
        """Test that reviews have venue info enrichment."""
        response = requests.get(f"{BASE_URL}/api/auth/my-reviews", headers=auth_headers)
        assert response.status_code == 200
        ...
        if data["total"] > 0:
            ...
            assert "venue_name" in review, "Review should have venue_name"
            assert "venue_image" in review, "Review should have venue_image"
            assert "rating" in review, "Review should have rating"
            print(f"PASS: Review has venue enrichment - venue: {review.get('venue_name')}, rating: {review.get('rating')}")
        else:
            print("SKIP: No reviews to verify enrichment")
...
class TestEndpointIntegration:
    ...
    def test_data_consistency(self, auth_headers):
        ...
        rec_response = requests.get(f"{BASE_URL}/api/auth/recommended-venues", headers=auth_headers)
        ...
        print(f"Data summary: {rec_data['total']} recommended venues, {bookings_data['total']} bookings, {payments_data['total']} payments, {invoices_data['total']} invoices")
