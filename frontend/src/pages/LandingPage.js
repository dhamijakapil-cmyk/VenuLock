...
function Reveal({ children, className = '', delay = 0 }) {
  ...
  return (
    <div ref={ref} className={`transition-all duration-[800ms] ease-[cubic-bezier(0.25,0.46,0.45,0.94)] ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'} ${className}`}
          ...
          {options.map(opt => {
            ...
            return (
              <button key={v} onClick={() => onSelect(v === value ? '' : v)}
                className={`w-full text-left px-4 py-3 text-[14px] transition-colors ${value === v ? 'bg-[#C8A960] text-[#0C0C0C] font-semibold' : 'text-white/45 hover:bg-white/[0.04] hover:text-white/65'}`}
...
function InlineField({ label, value, placeholder, icon: Icon, options, isOpen, onToggle, onSelect, testId, hasBorder = true }) {
  return (
    <div className={`relative flex-1 min-w-0 ${hasBorder ? 'border-r border-[#EBEBEB]' : ''}`} data-dropdown>
      <button onClick={onToggle} data-testid={testId}
        className={`w-full flex items-center gap-3.5 px-5 py-0 h-[72px] text-left transition-colors duration-100 ${isOpen ? 'bg-[#F7F7F5]' : 'hover:bg-[#FAFAF8]'}`}>
        <Icon className="w-4 h-4 text-[#BFBFBF] flex-shrink-0" strokeWidth={1.5} />
        <div className="flex-1 min-w-0">
          <div className="text-[10px] font-semibold uppercase tracking-[0.1em] text-[#BFBFBF] mb-[2px]">{label}</div>
          <div className={`text-[14px] truncate ${value ? 'text-[#111] font-medium' : 'text-[#D0D0D0]'}`}>{value || placeholder}</div>
        </div>
        <ChevronDown className={`w-3.5 h-3.5 text-[#D0D0D0] transition-transform duration-150 flex-shrink-0 ${isOpen ? 'rotate-180' : ''}`} />
      </button>
      {isOpen && (
        <div className="absolute top-full left-0 min-w-[240px] mt-1 bg-white border border-[#E5E5E5] shadow-[0_12px_40px_rgba(0,0,0,0.1)] z-50 max-h-64 overflow-y-auto">
          {options.map(opt => {
            const v = typeof opt === 'string' ? opt : opt.value;
            const l = typeof opt === 'string' ? opt : opt.label;
            return (
              <button key={v} onClick={() => onSelect(v === value ? '' : v)}
                className={`w-full text-left px-5 py-3 text-[14px] transition-colors ${value === v ? 'bg-[#0C0C0C] text-white font-medium' : 'text-[#555] hover:bg-[#F7F7F5]'}`}
                data-testid={`${testId}-option-${v.toLowerCase().replace(/[\s\/]+/g, '-')}`}>{l}</button>
            );
          })}
        </div>
  ...
  useEffect(() => {
    fetch(`${API_URL}/api/venues/cities`).then(r => r.json()).then(data => {
      if (Array.isArray(data) && data.length > 0) {
        setCitiesData(data);
        const DS = ['South Delhi', 'North Delhi', 'West Delhi', 'East Delhi'];
        setCityNames(data.flatMap(c => c.city === 'Delhi' ? DS : [c.city]));
      }
    }).catch(() => {});
  }, []);
    ...
    <div className="min-h-screen bg-white">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 lg:hidden bg-[#0A0A0A]" data-testid="mobile-header">
        <div className="flex items-center justify-between px-5 h-[56px]">
          <button onClick={() => navigate('/')} className="flex items-baseline gap-0" data-testid="logo-btn">
      ...
      {/* ══ DESKTOP HEADER ══ */}
      <header className="hidden lg:block sticky top-0 z-[9999] bg-[#0A0A0A]/95 backdrop-blur-md" data-testid="main-header">
        <div className="max-w-[1200px] mx-auto px-10 flex h-[64px] items-center justify-between">
          <button onClick={() => navigate('/')} className="flex items-baseline" data-testid="desktop-logo-btn">
            <span className="font-serif italic text-[22px] font-medium text-white tracking-[-0.01em]">Venu</span>
            <span className="font-serif italic text-[22px] font-medium text-[#C8A960] tracking-[-0.01em]">Lock</span>
          </button>
          <div className="flex items-center gap-10">
            ...
            <button onClick={() => navigate('/register')}
              className="text-[11px] font-bold text-[#0A0A0A] px-6 py-2.5 bg-[#C8A960] hover:bg-[#BF9F52] transition-colors duration-200 tracking-[0.06em] uppercase"
      ...
      <section data-testid="hero-section">
        ...
        <div className="lg:hidden pt-[56px]">
          <div className="bg-[#0A0A0A] px-5 pt-16 pb-20 relative overflow-hidden noise-overlay">
            ...
            <div className="relative z-10 text-center">
              ...
              <p className="text-white/30 text-[15px] leading-[1.65] max-w-[280px] mx-auto">
                ...
                <div className="space-y-3.5" data-testid="nearby-panel">
                  ...
                  {!geoCoords && !geoLoading && !geoError && (
                    <button onClick={handleGetLocation} className="w-full flex items-center justify-center gap-2 py-4 border border-dashed border-[#C8A960]/25 text-[#C8A960] text-sm font-medium hover:bg-[#C8A960]/5 transition-colors" data-testid="get-location-btn">
                      <Navigation className="w-4 h-4" strokeWidth={1.8} /> Enable Location
                    </button>
                  )}
                  {(geoCoords || geoLoading) && !geoError && (
                    <>
        ...
        <div className="hidden lg:block">
          <div className="bg-[#0A0A0A] pt-32 pb-40 relative noise-overlay">
            <div className="absolute inset-0 bg-[radial-gradient(ellipse_60%_50%_at_50%_40%,rgba(200,169,96,0.03)_0%,transparent_60%)]" />
            <div className="max-w-[800px] mx-auto px-10 text-center relative z-10">
              <h1 className="text-[7rem] font-medium leading-[0.92] tracking-[-0.035em] text-white font-serif" data-testid="hero-headline">
                We Talk.<br /><span className="text-[#C8A960]" style={{ textShadow: '0 0 80px rgba(200,169,96,0.1)' }}>You Lock.</span>
              </h1>
              <p className="text-[17px] leading-[1.6] max-w-[400px] mx-auto text-white/30 mt-8">
      ...
      {/* ══ DESKTOP SEARCH BAR (Inline Horizontal) ══ */}
      <section className="hidden lg:block bg-[#FAFAF9]" data-testid="search-section">
        <div className="max-w-[720px] mx-auto px-10 -mt-[36px] relative z-20 pb-10">
          {/* Mode toggle */}
          <div className="flex items-center justify-center gap-4 mb-4 text-[12px]">
            <button onClick={() => switchMode('city')}
              className={`transition-colors duration-150 ${searchMode === 'city' ? 'text-[#111] font-semibold' : 'text-[#BFBFBF] hover:text-[#888]'}`}
              data-testid="desktop-mode-city">Search by City</button>
            <span className="text-[#E0E0E0]">|</span>
            <button onClick={() => switchMode('nearby')}
              className={`transition-colors duration-150 ${searchMode === 'nearby' ? 'text-[#111] font-semibold' : 'text-[#BFBFBF] hover:text-[#888]'}`}
              data-testid="desktop-mode-nearby">Use My Location</button>
          </div>

          {/* Search bar */}
          <div className="bg-white border border-[#E5E5E5] shadow-[0_4px_24px_rgba(0,0,0,0.06),0_1px_3px_rgba(0,0,0,0.04)] flex items-stretch" data-testid="desktop-search-bar">
            {searchMode === 'city' ? (
              ...
              <div className="relative flex-1 min-w-0 border-r border-[#EBEBEB]" data-dropdown>
                {geoCoords ? (
                  <div className="flex items-center gap-3.5 px-5 h-[72px]">
                    <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse flex-shrink-0" />
                    <div><div className="text-[10px] font-semibold uppercase tracking-[0.1em] text-[#BFBFBF] mb-[2px]">Location</div><div className="text-[14px] font-medium text-emerald-600">Detected</div></div>
                  </div>
                ) : geoLoading ? (
                  <div className="flex items-center gap-3.5 px-5 h-[72px]">
                    <Loader2 className="w-4 h-4 animate-spin text-[#C8A960] flex-shrink-0" />
                    <div><div className="text-[10px] font-semibold uppercase tracking-[0.1em] text-[#BFBFBF] mb-[2px]">Location</div><div className="text-[14px] text-[#999]">Detecting...</div></div>
                  </div>
                ) : geoError ? (
                  <div className="flex items-center gap-3.5 px-5 h-[72px]"><div className="text-[14px] text-amber-600">{geoError}</div></div>
                ) : (
                  <button onClick={handleGetLocation} className="flex items-center gap-3.5 px-5 h-[72px] w-full text-left hover:bg-[#FAFAF8] transition-colors" data-testid="desktop-get-location-btn">
                    <Navigation className="w-4 h-4 text-[#C8A960] flex-shrink-0" strokeWidth={1.5} />
                    <div><div className="text-[10px] font-semibold uppercase tracking-[0.1em] text-[#BFBFBF] mb-[2px]">Location</div><div className="text-[14px] text-[#C8A960] font-medium">Enable Access</div></div>
                  </button>
                )}
              </div>
            )}
            ...
            <button onClick={handleExplore} disabled={searchMode === 'nearby' && !geoCoords}
              className="px-8 bg-[#0A0A0A] text-white flex items-center gap-2.5 text-[12px] font-bold tracking-[0.06em] uppercase whitespace-nowrap hover:bg-[#1A1A1A] disabled:opacity-30 transition-colors duration-150 flex-shrink-0"
              data-testid="desktop-explore-venues-btn">
              Find Venue <ArrowRight className="w-4 h-4" strokeWidth={2} />
            </button>
          </div>

          {/* Trust line */}
          <div className="mt-6 text-center text-[12px] text-[#C0C0C0]" data-testid="desktop-trust-strip">
            500+ Verified Venues
            <span className="inline-block w-[3px] h-[3px] bg-[#D5D5D5] rounded-full mx-4 align-middle" />
            Transparent Pricing
            <span className="inline-block w-[3px] h-[3px] bg-[#D5D5D5] rounded-full mx-4 align-middle" />
            End-to-End Support
            <span className="inline-block w-[3px] h-[3px] bg-[#D5D5D5] rounded-full mx-4 align-middle" />
            <button onClick={() => navigate('/venues/search')} className="text-[#111] font-medium underline underline-offset-3 decoration-[#111]/15 hover:decoration-[#111]/40 transition-colors" data-testid="desktop-browse-all-link">Browse All</button>
          </div>
        </div>
      </section>

      {/* ══ WHY VENULOCK ══ */}
      <section className="py-24 lg:py-32 bg-[#FAFAF9]" data-testid="why-choose-us">
        <div className="max-w-[1040px] mx-auto px-5 lg:px-10">
          <Reveal>
            <div className="text-center mb-16 lg:mb-20">
              <h2 className="text-[30px] lg:text-[40px] font-medium text-[#111] font-serif leading-[1.1]">Why VenuLock</h2>
              <p className="text-[15px] text-[#999] mt-4 max-w-[420px] mx-auto leading-[1.6]">Everything you need to find and book the right venue, without the runaround.</p>
            </div>
          </Reveal>
          <div className="grid sm:grid-cols-3 gap-px bg-[#EBEBEB]">
            {WHY_CHOOSE.map((item, i) => (
              <Reveal key={item.title} delay={i * 80}>
                <div className="bg-white p-8 lg:p-10" data-testid={`why-card-${item.title.toLowerCase().replace(/\s/g, '-')}`}>
                  <div className="w-11 h-11 bg-[#0A0A0A] flex items-center justify-center mb-6">
                    <item.icon className="w-[18px] h-[18px] text-white" strokeWidth={1.5} />
                  </div>
                  <h3 className="text-[17px] font-semibold text-[#111] mb-2.5 font-sans">{item.title}</h3>
                  <p className="text-[14px] leading-[1.7] text-[#888]">{item.desc}</p>
                </div>
              </Reveal>
            ))}
          </div>
      ...
      {/* ══ SOCIAL PROOF ══ */}
      <section className="py-16 lg:py-20 bg-[#0A0A0A]" data-testid="social-proof">
        <Reveal>
          <div className="max-w-[720px] mx-auto px-5 lg:px-10 grid grid-cols-3 text-center">
            {[
              { val: '500+', label: 'Venues Listed' },
              { val: '100%', label: 'Verified Partners' },
              { val: 'End-to-End', label: 'Booking Support' },
            ].map(s => (
              <div key={s.label}>
                <div className="text-[36px] lg:text-[48px] font-medium text-white leading-none font-serif">{s.val}</div>
                <div className="text-[11px] text-white/25 font-medium tracking-[0.1em] uppercase mt-2">{s.label}</div>
              </div>
            ))}
          </div>
        </Reveal>
      ...
      {/* ══ HOW IT WORKS ══ */}
      <section className="py-24 lg:py-32 bg-white" id="how-it-works" data-testid="how-it-works">
        <div className="max-w-[1040px] mx-auto px-5 lg:px-10">
          <Reveal>
            <div className="text-center mb-16 lg:mb-20">
              <h2 className="text-[30px] lg:text-[40px] font-medium text-[#111] font-serif leading-[1.1]">How It Works</h2>
            </div>
          </Reveal>
            ...
            {STEPS.map((s, i) => (
              <Reveal key={s.num} delay={i * 100}>
                <div className="text-center sm:text-left">
                  <div className="text-[56px] lg:text-[72px] font-medium text-[#F0F0F0] leading-none font-serif select-none">{s.num}</div>
                  <h3 className="text-[17px] font-semibold text-[#111] mt-2 mb-2 font-sans">{s.title}</h3>
                  <p className="text-[14px] leading-[1.7] text-[#888]">{s.desc}</p>
                </div>
              </Reveal>
            ))}
          </div>
      ...
      {/* ══ VENUE EXPERTS ══ */}
      <section className="py-24 lg:py-32 bg-[#FAFAF9] border-t border-[#F0F0F0]" data-testid="top-performers-section">
        <div className="max-w-[1040px] mx-auto px-5 lg:px-10">
          <Reveal>
            <div className="text-center mb-14">
              <h2 className="text-[30px] lg:text-[40px] font-medium text-[#111] font-serif leading-[1.1]">Meet Your Venue Experts</h2>
              <p className="text-[15px] text-[#999] mt-4">Dedicated managers who guide you from search to booking.</p>
            </div>
          </Reveal>
          {topPerformers.length > 0 ? (
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-px bg-[#EBEBEB]">
              {topPerformers.map((rm, idx) => (
                <Reveal key={rm.user_id} delay={idx * 60}>
                  <div className="bg-white p-7" data-testid={`top-performer-card-${idx}`}>
                    <div className="flex items-start gap-4 mb-4">
                      <img src={rm.picture || `https://ui-avatars.com/api/?name=${encodeURIComponent(rm.name)}&background=0A0A0A&color=fff&size=48`}
                        alt={rm.name} className="w-11 h-11 rounded-full object-cover flex-shrink-0" />
                      <div className="flex-1 min-w-0">
            ...
            <div className="text-center py-12 text-[#BFBFBF] text-sm">Loading venue experts...</div>
          )}
        </div>
      </section>

      {/* ══ PLATFORM CAPABILITIES ══ */}
      <section className="py-24 lg:py-32 bg-white border-t border-[#F0F0F0]" data-testid="platform-advantage">
        <div className="max-w-[1040px] mx-auto px-5 lg:px-10">
          <Reveal>
            <div className="text-center mb-16 lg:mb-20">
              <h2 className="text-[30px] lg:text-[40px] font-medium text-[#111] font-serif leading-[1.1]">Platform Capabilities</h2>
              <p className="text-[15px] text-[#999] mt-4">Built for modern event planning at scale.</p>
            </div>
          </Reveal>
          <div className="grid sm:grid-cols-2 gap-px bg-[#EBEBEB]">
            {CAPABILITIES.map((cap, i) => (
              <Reveal key={cap.title} delay={i * 60}>
                <div className="bg-white p-8 lg:p-10" data-testid="capability-card">
                  <cap.icon className="h-5 w-5 mb-5 text-[#0A0A0A]" strokeWidth={1.5} />
                  <h3 className="text-[16px] font-semibold text-[#111] mb-2 font-sans">{cap.title}</h3>
                  <p className="text-[14px] leading-[1.7] text-[#888]">{cap.desc}</p>
                </div>
              </Reveal>
            ))}
          </div>
      ...
      {/* ══ BROWSE BY CITY ══ */}
      <section className="py-24 lg:py-32 bg-[#FAFAF9] border-t border-[#F0F0F0]" data-testid="city-coverage">
        <div className="max-w-[1040px] mx-auto px-5 lg:px-10">
          <Reveal>
            <div className="flex items-end justify-between mb-10">
              <h2 className="text-[30px] lg:text-[40px] font-medium text-[#111] font-serif leading-[1.1]">Browse by City</h2>
      ...
      {/* ══ WORK WITH US ══ */}
      <section className="py-24 lg:py-32 bg-white border-t border-[#F0F0F0]" data-testid="work-with-us">
        <div className="max-w-[1040px] mx-auto px-5 lg:px-10">
          <Reveal>
            <div className="text-center mb-14">
              <h2 className="text-[30px] lg:text-[40px] font-medium text-[#111] font-serif leading-[1.1]">Grow With VenuLock</h2>
              <p className="text-[15px] text-[#999] mt-4">Join our ecosystem as a venue partner or event management company.</p>
            </div>
          </Reveal>
          <div className="grid sm:grid-cols-2 gap-5 lg:gap-6">
            <Reveal delay={0}>
              <div className="bg-[#0A0A0A] p-8 lg:p-10 h-full" data-testid="list-venue-cta">
                <Building2 className="h-6 w-6 text-[#C8A960] mb-6" strokeWidth={1.2} />
                <h3 className="text-[22px] font-medium text-white mb-3 font-serif">List Your Venue</h3>
                <p className="text-white/35 text-[14px] mb-8 leading-[1.7]">Get qualified leads from event planners. We handle discovery and follow-up — you focus on great events.</p>
            ...
            <Reveal delay={80}>
              <div className="bg-white border border-[#EBEBEB] p-8 lg:p-10 h-full" data-testid="partner-cta">
                <Handshake className="h-6 w-6 text-[#0A0A0A] mb-6" strokeWidth={1.2} />
                <h3 className="text-[22px] font-medium text-[#111] mb-3 font-serif">Partner With Us</h3>
                <p className="text-[#888] text-[14px] mb-8 leading-[1.7]">Join our network for premium venue access, co-marketing, and a shared customer pipeline.</p>
                <ul className="space-y-2.5 mb-8">
      ...
      {/* ══ FINAL CTA ══ */}
      <section className="py-24 lg:py-32 bg-[#0A0A0A]" data-testid="final-cta">
        <Reveal>
          <div className="max-w-[560px] mx-auto px-5 lg:px-10 text-center">
            <h2 className="text-[30px] lg:text-[40px] font-medium text-white font-serif leading-[1.1]">Ready to lock your venue?</h2>
            <p className="text-[15px] text-white/25 mt-4 mb-10">Get started in under 2 minutes. Free, no commitment.</p>
      ...
      {/* ══ FOOTER ══ */}
      <footer className="py-14 bg-[#0A0A0A] border-t border-white/[0.04]" data-testid="main-footer">
        <div className="max-w-[1040px] mx-auto px-5 lg:px-10">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-8 mb-12">
            ...
            <div>
              <h4 className="text-[11px] uppercase tracking-[0.1em] font-semibold text-white/25 mb-4">Platform</h4>
            ...
            <div>
              <h4 className="text-[11px] uppercase tracking-[0.1em] font-semibold text-white/25 mb-4">Company</h4>
              <ul className="space-y-2.5">
            ...
            <div>
              <h4 className="text-[11px] uppercase tracking-[0.1em] font-semibold text-white/25 mb-4">Cities</h4>
              <ul className="space-y-2.5">
