      ...
      <section className="relative bg-[#1A1A1A]" data-testid="hero-section">
        <div className="absolute inset-0 overflow-hidden will-change-transform" style={{ transform: `translateY(${heroParallax}px)` }}>
          {HERO_IMAGES.map((src, i) => (
            <img key={src} src={src} alt="" className="absolute inset-0 w-full h-full object-cover" style={{ opacity: i === heroImageIndex ? 0.55 : 0, transform: `scale(${i === heroImageIndex ? 1.08 : 1.02})`, transition: 'opacity 1.8s ease-in-out, transform 5s ease-out' }} loading={i === 0 ? 'eager' : 'lazy'} />
          ))}
          <div className="absolute inset-0 bg-gradient-to-b from-[#1A1A1A]/30 via-[#1A1A1A]/15 to-[#1A1A1A]/85" />
