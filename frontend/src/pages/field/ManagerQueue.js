...
function posture(completeness) {
  if (!completeness) return { mandatory: 'missing', media: 'missing', commercial: 'missing' };
  return {
    mandatory: completeness.mandatory?.complete ? 'complete' : 'incomplete',
    media: completeness.media?.complete ? 'complete' : completeness.media?.count > 0 ? 'weak' : 'missing',
    commercial: completeness.commercial?.complete ? 'complete' : completeness.commercial?.filled > 0 ? 'partial' : 'missing',
  };
}
...
export default function ManagerQueue() {
  ...
  const [tab, setTab] = useState('awaiting_manager_approval');
  ...
  return (
    <div className="min-h-screen bg-[#FAFBF9]" data-testid="manager-queue">
      {/* Header */}
      <div className="bg-[#0B0B0D] px-4 pt-3 pb-5" style={{ paddingTop: 'calc(env(safe-area-inset-top, 8px) + 12px)' }}>
        <button onClick={() => navigate('/team/dashboard')} className="flex items-center gap-1.5 text-white/50 mb-2" data-testid="mgr-back">
          <ArrowLeft className="w-4 h-4" />
          <span className="text-[11px]" style={sans}>Dashboard</span>
        </button>
        <div className="flex items-center gap-2 mb-1">
          <Shield className="w-4 h-4 text-[#D4B36A]" />
          <p className="text-[10px] text-white/40 uppercase tracking-[0.2em]" style={sans}>Final Gate</p>
        </div>
        <h1 className="text-[20px] font-bold text-white" style={sans}>Manager Approval</h1>
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-1.5">
                        ...
                        {hasSendBackHistory && (
                          <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 bg-amber-50 rounded text-[8px] font-bold text-amber-600 flex-shrink-0" style={sans}>
                            <AlertTriangle className="w-2.5 h-2.5" /> REWORK
                          </span>
                        )}
                      </div>
